...
public:
  ...
  ZmqBusHelper(const std::string &pubEndpoint, const std::string &subEndpoint)
      : context(1), pub(context, zmq::socket_type::pub),
  ...
  void publish(const std::string &topic, const std::string &payload) {
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(10)); // Tiny sleep to ensure distribution
  }
...
TEST_F(NetworkManagerTest, Instantiation) {
  EXPECT_NO_THROW({
    rtypeEngine::NetworkManager nm("tcp://127.0.0.1:5555",
                                   "tcp://127.0.0.1:5556");
  });
}

TEST_F(NetworkManagerTest, BindDoesNotCrash) {
  rtypeEngine::NetworkManager nm("tcp://127.0.0.1:5557",
                                 "tcp://127.0.0.1:5558");
  nm.init();
...
TEST_F(NetworkManagerTest, ConnectDoesNotCrash) {
  rtypeEngine::NetworkManager nm("tcp://127.0.0.1:5559",
                                 "tcp://127.0.0.1:5560");
  nm.init();
...
TEST_F(NetworkManagerTest, MessageQueueOperations) {
  rtypeEngine::NetworkManager nm("tcp://127.0.0.1:5569",
                                 "tcp://127.0.0.1:5570");
  nm.init();
...
TEST_F(NetworkManagerTest, SendToClientDoesNotCrashWithInvalidId) {
  rtypeEngine::NetworkManager nm("tcp://127.0.0.1:5565",
                                 "tcp://127.0.0.1:5566");
  nm.init();
  nm.bind(4250);
...
TEST_F(NetworkManagerTest, BroadcastDoesNotCrashWithNoClients) {
  rtypeEngine::NetworkManager nm("tcp://127.0.0.1:5567",
                                 "tcp://127.0.0.1:5568");
  nm.init();
  nm.bind(4251);
...
TEST_F(NetworkManagerTest, SendWithoutConnection) {
  rtypeEngine::NetworkManager nm("tcp://127.0.0.1:5594",
                                 "tcp://127.0.0.1:5595");
  nm.init();
...
TEST_F(NetworkManagerTest, ReBind) {
  rtypeEngine::NetworkManager nm("tcp://127.0.0.1:5596",
                                 "tcp://127.0.0.1:5597");
  nm.init();
...
TEST_F(NetworkManagerTest, EndToEndCommunication) {
  rtypeEngine::NetworkManager server("tcp://127.0.0.1:5571",
                                     "tcp://127.0.0.1:5572");
  rtypeEngine::NetworkManager client("tcp://127.0.0.1:5573",
                                     "tcp://127.0.0.1:5574");
...
TEST_F(NetworkManagerTest, MultipleClientsBroadcast) {
  rtypeEngine::NetworkManager server("tcp://127.0.0.1:5583",
                                     "tcp://127.0.0.1:5584");
  rtypeEngine::NetworkManager client1("tcp://127.0.0.1:5585",
                                      "tcp://127.0.0.1:5586");
  rtypeEngine::NetworkManager client2("tcp://127.0.0.1:5587",
                                      "tcp://127.0.0.1:5588");
...
TEST_F(NetworkManagerTest, DisconnectCleansUpClients) {
  rtypeEngine::NetworkManager server("tcp://127.0.0.1:5590",
                                     "tcp://127.0.0.1:5591");
  rtypeEngine::NetworkManager client("tcp://127.0.0.1:5592",
                                     "tcp://127.0.0.1:5593");
  ...
  if (server.getConnectedClients().size() == 1) {
    server.disconnect();
    std::this_thread::sleep_for(
        std::chrono::milliseconds(100)); // Allow async disconnect to process
...
TEST_F(NetworkManagerTest, Aggressive_MalformedUdpPacket) {
    ...
    rtypeEngine::NetworkManager nm("tcp://127.0.0.1:5600",
"tcp://127.0.0.1:5601");
    ...
    nm.init();
    nm.bind(4248);
    std::this_thread::sleep_for(std::chrono::milliseconds(200)); // Wait for
bind
...
asio::ip::udp::endpoint(asio::ip::udp::v4(), 0)); asio::ip::udp::endpoint
target(asio::ip::address::from_string("127.0.0.1"), 4248);
...
std::chrono::milliseconds(2000)) { nm.loop(); if
(bus.hasReceived("InvalidPacket")) { found = true; break;
        }

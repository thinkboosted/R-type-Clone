...
AApplication::AApplication()
    : _zmqContext(1), _xpubSocket(nullptr), _xsubSocket(nullptr),
      _publisher(nullptr), _subscriber(nullptr), _isBrokerActive(false),
      _isServerMode(false) {}

AApplication::~AApplication() { cleanupMessageBroker(); }

// New method to configure and initialize the ZMQ broker/connections based on
// role
void AApplication::setupBroker(const std::string &baseEndpoint, bool isServer) {
  ...
  if (_pubBrokerEndpoint.find("tcp://") != 0 &&
      _pubBrokerEndpoint.find("ipc://") != 0 &&
      _pubBrokerEndpoint.find("inproc://") != 0) {
    _pubBrokerEndpoint = "tcp://" + _pubBrokerEndpoint;
  }
  if (_subBrokerEndpoint.find("tcp://") != 0 &&
      _subBrokerEndpoint.find("ipc://") != 0 &&
      _subBrokerEndpoint.find("inproc://") != 0) {
  ...
  if (_isServerMode) {
    ...
    try {
      _xpubSocket =
          std::make_unique<zmq::socket_t>(_zmqContext, zmq::socket_type::xpub);
      _xsubSocket =
          std::make_unique<zmq::socket_t>(_zmqContext, zmq::socket_type::xsub);
      _publisher =
          std::make_unique<zmq::socket_t>(_zmqContext, zmq::socket_type::pub);
      _subscriber =
          std::make_unique<zmq::socket_t>(_zmqContext, zmq::socket_type::sub);
      ...
      if (_subBrokerEndpoint.find(":1") != std::string::npos &&
          baseEndpoint.find(":0") != std::string::npos) {
        // If base was port 0, sub endpoint calculation +1 is
        // invalid/meaningless before bind. Just bind sub to random port too.
      ...
      _subscriber->set(zmq::sockopt::subscribe,
                       ""); // Subscribe to all for internal processing

      _isBrokerActive = true;

      _proxyThread = std::thread([this]() {
        try {
          zmq::proxy(zmq::socket_ref(*_xsubSocket),
                     zmq::socket_ref(*_xpubSocket));
        } catch (const zmq::error_t &e) {
          // ETERM means context was terminated, which is expected during
          // shutdown
    ...
    // Client Mode: This AApplication instance connects to an external broker
    // (the server's)
    try {
      _publisher =
          std::make_unique<zmq::socket_t>(_zmqContext, zmq::socket_type::pub);
      ...
      _subscriber->set(zmq::sockopt::subscribe,
                       ""); // Subscribe to all for client-side processing

      _isBrokerActive = true; // Broker is active via connection
    } catch (const zmq::error_t &e) {
      std::cerr << "Failed to setup client message connections: " << e.what()
                << std::endl;
      throw;
    }
  }
...
// cleanupMessageBroker: Adjusted to handle potentially null
// _xpubSocket/_xsubSocket
...
void AApplication::addModule(const std::string &modulePath,
                             const std::string &pubEndpoint,
                             const std::string &subEndpoint) {
  _modules.push_back(
      _modulesManager->loadModule(modulePath, pubEndpoint, subEndpoint));
}
...
void AApplication::sendMessage(const std::string &topic,
                               const std::string &message) {
  if (!_publisher || !_isBrokerActive) {
    std::cerr << "Attempt to send message without active publisher."
              << std::endl;
    return;
  }
...
std::string AApplication::getMessage(const std::string &topic) {
  if (!_subscriber || !_isBrokerActive) {
    // std::cerr << "Attempt to get message without active subscriber." <<
    // std::endl;
    return "";
  }
  ...
  std::string fullMessage(static_cast<char *>(zmqMessage.data()),
                          zmqMessage.size());

  // Check if the message actually matches the expected topic
  // Note: This logic assumes topic is at the beginning, then a space.
  if (fullMessage.find(topic) == 0 && (fullMessage.length() == topic.length() ||
                                       fullMessage[topic.length()] == ' ')) {
...
void AApplication::unsubscribe(const std::string &topic) {
  if (!_subscriber || !_isBrokerActive) {
    std::cerr << "Attempt to unsubscribe without active subscriber."
              << std::endl;
    return;
  }

...
Rtype::Rtype(const std::string &endpoint, const std::vector<std::string>& args) : AApplication() {
  ...
  // If we are a client, we must NOT bind to the same ports as the game server (if running locally).
  // Use ephemeral ports (0) to let the OS assign free ports for our internal bus.
...
void Rtype::init() {
  ...
  subscribe("NetworkStatus", [this](const std::string& payload) {
    ...
    if (_args[0] == "client" && payload.find("Connected:") != std::string::npos) {
  ...
  subscribe("NetworkMessage", [this](const std::string& payload) {
    ...
    if (!firstWord.empty() && std::isdigit(firstWord[0])) { // Message starts with a client ID (server-side format)
        try {
            ...
            if (ss >> actualMessageType) { // Read next word as message type
                ...
                actualContent = actualContent.substr(actualContent.find_first_not_of(' ', 0)); // Trim leading space
            } else { // Only clientId was present, no messageType
                actualMessageType = ""; // No message type, only client ID
                actualContent = "";
            }
        } catch (const std::invalid_argument& e) { // If stoul fails, firstWord was not a client ID
    ...
    } else { // Message does not start with a client ID (e.g. "PONG 1" or "PING")
    ...
    if (actualMessageType == "CLIENT_HELLO") {
        ...
        try {
            ...
            std::cerr << "[Client] Error parsing PONG content: " << e.what() << std::endl;
            std::cout << "[Client] Received PONG (no valid client ID in content: \"" << actualContent << "\")" << std::endl;
        }
    } else {
        std::cout << "[Game] Unknown NetworkMessage type: " << actualMessageType << " content: \"" << actualContent << "\"" << std::endl;
    }
  });
}
...
void Rtype::loop() {
    if (!_networkInitDone && !_args.empty()) {
        ...
        if (_args[0] == "server" && _args.size() >= 2) {
        ...
        } else if (_args[0] == "client" && _args.size() >= 3) { // If client, request connect
            ...
            // Initial "HelloFromClient" message has been removed here, as CLIENT_HELLO is used instead.
        }

...
function MenuSystem.renderMenu()
    ...
    -- SAFETY: Check for existing elements and clear them just in case
...
function MenuSystem.renderPauseMenu()
    ...
    -- SAFETY: Always clear existing elements before rendering to avoid stacking/leaks
    for _, id in ipairs(menuElements) do
        ECS.destroyUI(id)
    end
...
function MenuSystem.onKeyPressed(key)
    ...
    if key == "ESCAPE" then
        ...
        print(string.format("[MenuSystem] ESC Timer: %.4f Last: %.4f Diff: %.4f Cooldown: %.4f", currentTime, lastToggleTime, currentTime - lastToggleTime, toggleCooldown))

        if currentTime - lastToggleTime < toggleCooldown then
            print("[MenuSystem] ESC Debounced")
            return
        end
        lastToggleTime = currentTime

        print(string.format("[MenuSystem] ESC Logic - Paused: %s, Running: %s, Rendered: %s", tostring(isPaused), tostring(ECS.isGameRunning), tostring(isMenuRendered)))
